...
if platform.system() == 'Windows':
  ...
  from src.Interfaces import drawTargetArea
...
class RafikiFrame(wx.Frame):
  ...
  def runScript(self, event):
      if self.running == False:
        ...
        self.SE.runscript(script=script, wnd=self.target)
        self.running = True
  ...
  def windowArea(self,event):
      """ DIRTY - But this one was killing me!
          @todo: Prettify... maybe some day...
      """
      hwnd = 0
      maxTimer = 25
      ...
      if platform.system() == 'Windows':
          ...
          start = time.time()
          while True:
              state_ctrl = winapi.GetAsyncKeyState(conn.VK_CONTROL)
              state_t    = winapi.GetAsyncKeyState(ord('W')) #W for Window
              if state_t != 0 and state_ctrl != 0:
                break

              pos = winapi.GetCursorPos()
              hwnd = winapi.WindowFromPoint(pos)
              frame = winapi.GetWindowRect(hwnd)
              ...
              boxWidth = ex - sx
              boxHeight = ey - sy  
              dc.DrawRectangle(sx-1, sy, boxWidth+2, boxHeight+2)
              time.sleep(0.12)
              dc.DrawRectangle(sx-1, sy, boxWidth+2, boxHeight+2)
          ...
          #@todo: wx.Yield()-integration
          hwnd = drawTargetArea(maxTimer, 3)

      else:
        self.writeLn("[Unsupported OS] Target can not be selected")  
        event.Skip()
        return 

      self.target = hwnd      
      self.writeLn("New target area: " + str(hwnd))  
      event.Skip()
...
class FileMenu:
    ...
    def __init__(self, rafiki):
        ''' File menu... '''
        filem = wx.Menu()
        filem.Append(ID_NEW,    "&New project\tCtrl+N", "New project")
        filem.Append(ID_NEWTAB, "&Add tab\tCtrl+T",     "New project")
        filem.Append(ID_OPEN,   "&Open\tCtrl+O",        "Open a project")
        filem.Append(ID_RECENT, "Open &recent",         "Reopen a poject")
        filem.Append(ID_SAVE,   "&Save\tCtrl+S",        "Save project")
        filem.Append(ID_SAVEAS, "Save &as...",          "Save project as...")
        filem.AppendSeparator()
        filem.Append(ID_EXIT,   "E&xit\tCtrl+Q", "Terminate the program")
        ...
        tools.Append(ID_PRTSCN,    "&Take Screenshoot", "Take a screenshot!")
        ...
        tools.AppendSeparator()
        tools.Append(ID_NUMBERS,   "&Calculator", "A simple calculator")
...
class Toolbar:
    ...
    def __init__(self, rafiki):
        ...
        toolbar.AddSimpleTool(ID_WINDOW, wx.Image(ICONS + 'area.png',
            wx.BITMAP_TYPE_PNG).ConvertToBitmap(), 'Target window (Ctrl+W = select)', 
                                                   'Select target window - Select with Ctrl+W')
...
class ScriptEngine:
  ...
  def __runscript(self, script, wnd):
      ...
      argument.append(self.tempfile) # File
      argument.append(str(wnd)) # Handle

      #Xlib on linux seems to spit out this word...
...
'''~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
| Just a plain old and ugly "About"-dialog
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'''
class AboutFrame(wx.Dialog):
    ...
    def __init__(self, parent=None):
      ...
      html.SetPage("""
              ...
              Rafiki is also Open Source, released under the wxWindows licence.</p>
      ...
      sizer = wx.BoxSizer(wx.VERTICAL)
      sizer.Add(html, -1, flag=wx.EXPAND|wx.ALL, border=2)
      self.SetSizer(sizer)
