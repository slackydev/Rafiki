...
from .display import get_target_rect
...
def move(x,y, speed=12, rand=(0,0), skipclose=1):
    ...
    #---------------------------------------------------------------
    speed = speed if speed > 0 else 12
...
def clean_move(x,y, speed=1):
    ...
    while Current<1:
        ...
        if value!=0: Current += (6.0/value)
        else: Current += 1.0
        ex,ey = cX+(dX*Current), cY+(dY*Current)
